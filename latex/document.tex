...
\begin{document}
	...
	\author[Valdés, Vásquez, Gutierrez, Opitz Ulloa]{Simón Valdés Oliva \and Martín Vásquez \and Alejandro Gutierrez \and Mathias Opitz \and Gabriel Ulloa}
	...
	\begin{frame}{¿Qué es Rust?}
		...
		Su objetivo principal es permitir escribir software de bajo nivel, similar a C o C++, pero eliminando los errores de memoria que históricamente han causado la mayoría de las vulnerabilidades de seguridad.
	\end{frame}

	\begin{frame}{¿Qué problema viene a resolver?}
		\begin{alertblock}{Gestión de memoria}
			Aproximadamente el 70\% de las vulnerabilidades de seguridad en proyectos de software son errores de memoria.
		\end{alertblock}
		
		\vspace{0.3cm}
		
		Antes de la existencia de Rust, los desarrolladores tenían dos opciones:
		\begin{itemize}
			\item Usar \textbf{C o C++}: muy rápidos, pero el programador gestiona la memoria a mano y se equivoca.
			\item Usar lenguajes con \textbf{garbage collector} como Java o Go, que son más seguros, pero con pausas impredecibles en la ejecución.
		\end{itemize}
		
		\vspace{0.2cm}
		Rust ofrece \textbf{seguridad de memoria verificada por el compilador}, sin recolector de basura y con rendimiento comparable a C.
	\end{frame}
	...
	\begin{frame}{¿Compilado, interpretado o híbrido?}
		Rust es un lenguaje \textbf{compilado}. El código fuente se traduce directamente a código de máquina nativo.
	...
	\begin{frame}{Zero-cost abstractions}
		...
		\begin{itemize}
			\item Si escribimos de forma no óptima, el código compilado es igual de rápido que si lo hubiéramos escrito de forma más inteligente.
	...
	\begin{frame}{Gestión de memoria: Ownership}
	...
	\begin{frame}{Borrowing: prestar sin regalar}
		Rust permite \textbf{prestar} valores sin transferir la propiedad\footnote{Crear una referencia temporal a un dato. Mientras alguien lo lee, nadie puede modificarlo, y viceversa.}.
	...
	\begin{frame}[fragile]{Manejo de errores}
		Rust \textbf{no tiene exceptions}. Los errores se representan como valores que el programador debe manejar explícitamente:
...
fn main() {
	...
	\begin{frame}{¿Qué paradigmas soporta Rust?}
		...
		\vspace{0.2cm}
		\begin{itemize}
			\item \textbf{Paradigma imperativo:} variables mutables, bucles, control de flujo secuencial.
			\item \textbf{Paradigma funcional:} funciones como valores, inmutabilidad por defecto, encadenamiento de operaciones.
			\item \textbf{Paradigma orientado a objetos *:} permite encapsular datos y comportamiento con \texttt{struct} e \texttt{impl}, y definir interfaces con \texttt{trait}\footnote{Similar a una interfaz en Java: define un contrato de métodos que distintos tipos pueden implementar. Rust usa traits en vez de herencia para lograr polimorfismo.}. Pero no tiene herencia de clases. En su lugar, favorece la composición.
		\end{itemize}
	\end{frame}
	
	\begin{frame}[fragile]{Ejemplo: paradigma imperativo}
		Operación: $\sum_{i=1}^{10}i$. Muy similar a C.
		\begin{lstlisting}[style=rust]
fn main() {
    let mut suma = 0;
    for i in 1..=10 {
    ...
    println!("Suma de 1 a 10: {}", suma); // 55
		...
		Notar que la variable \texttt{suma} debe marcarse explícitamente como \texttt{mut}. En Rust, las variables son \textbf{inmutables por defecto}. 
	\end{frame}

	\begin{frame}[fragile]{Ejemplo: paradigma funcional}
		Iteradores, closures\footnote{Equivalentes a las funciones lambda de Haskell. En Rust se escriben con la sintaxis \texttt{|args| cuerpo}.} y encadenamiento de operaciones:
		\begin{lstlisting}[style=rust]
fn main() {
    let numeros = vec![1, 2, 3, 4, 5, 6];

    let pares_dobles: Vec<i32> = numeros.iter()
        .filter(|&&x| x % 2 == 0)  // solo para pares
    ...
    println!("{:?}", pares_dobles); // [4, 8, 12]
		...
		Se describe qué se quiere obtener, no cómo paso a paso. Las funciones \texttt{filter} y \texttt{map} reciben funciones anónimas como argumento.
	\end{frame}
...
impl Describible for Gato {
	...
	\begin{frame}{Herramientas que vienen con Rust}
		...
		\vspace{0.2cm}
		\begin{itemize}
			\item \textbf{Cargo:} gestor de paquetes y sistema de compilación.
			...
			\item \textbf{rust-analyzer:} servidor de lenguaje para editores de código.
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Los principios detrás de Rust}
		\begin{block}{Principio central}
			La comunidad de Rust confía en que el compilador atrapa la mayoría de los errores antes de ejecutar el programa. Si compila funciona, básicamente.
		\end{block}
		
		\vspace{0.3cm}
		Los pilares que guían el diseño del lenguaje son:
		\begin{itemize}
			\item \textbf{Seguridad:} El compilador no permite compilar código con errores de memoria.
			\item \textbf{Las abstracciones no deben costar rendimiento.} Lo que se escribe de forma elegante debe ejecutarse igual de rápido que lo escrito ``spaghetti''.
	...
	Es código que aprovecha las herramientas del lenguaje de la forma en que fueron pensadas.
		\vspace{0.2cm}
		\begin{columns}
			\column{0.5\textwidth}
			\textbf{Sí es:}
	...
	% ==========================================================
	\section{Análisis según Sebesta}
	...
	\begin{frame}{Costo}
		\begin{columns}
			...
			\textbf{Costos altos:}
			\begin{itemize}
				\item \textbf{Aprendizaje:} la curva es pronunciada, el compilador rechaza mucho código al principio.
				\item \textbf{Compilación:} más lenta que C, ya que el compilador hace un análisis profundo.
			...
			\textbf{Costos bajos:}
			\begin{itemize}
				\item \textbf{Ejecución:} rendimiento similar a C.
	...
	\begin{frame}{¿Debería existir otro criterio?}
		...
		\begin{itemize}
			...
			\item Un lenguaje que garantiza seguridad de memoria en compilación no puede evaluarse adecuadamente solo con el criterio de confiabilidad.
	...
	\begin{frame}{Rust hoy}
		Rust se usa principalmente en:
	...
	\begin{frame}{Linux Kernel}
		\begin{block}{Linux 7.0}
			Desde diciembre de 2025, Rust es un \textbf{lenguaje oficial} dentro del kernel de Linux.
		\end{block}
		
		\vspace{0.3cm}
		\begin{itemize}
			\item Hay drivers escritos en Rust, como controladores de red, el driver Android Binder, y drivers de GPU como el Apple AGX y el NVIDIA Nova.
	...
	\begin{frame}{Tauri: apps de escritorio}
		Tauri es un framework para crear aplicaciones de escritorio multiplataforma. Es la alternativa moderna a Electron\footnote{Usar tecnología web para programas de escritorio. Ej: VSCode, Discord, WhatsApp}.
		

			¿Cómo funciona?
			\begin{itemize}
				\item La UI se construye con tecnologías web.
				\item El backend se escribe en Rust.
				\item No se usa un navegador Chromium completo. Tauri usa el WebView que ya trae el sistema operativo.
			\end{itemize}

		Esto significa tener aplicaciones que consumen muchísimos menos recursos.
	...
	\begin{frame}{Discord}
			...
			\begin{itemize}
				\item Migraron partes esenciales de su backend de Go a Rust.
	...
	\begin{frame}{¿Por qué elegir Rust?}
		\begin{columns}
			...
			\textbf{Ventajas}
			\begin{itemize}
				\item Rendimiento similar a C y C++.
			...
			\textbf{Desafíos}
			\begin{itemize}
				\item Alta curva de aprendizaje.
